@tool
class_name CardAbility_Target
extends CardAbility

@export var bonus_type: BonusType
@export var count: int = 1

func get_ability_name(markedup: bool = false, short: bool = false) -> String:
	var count_str := (str(count) + ' ') if count > 1 else ''
	if markedup:
		if short:
			return tr('Target', 'ABILITY') + ' %s[img width=1.25em height=1.25em]%s[/img]' % [count_str, bonus_type.icon.resource_path]
		else:
			return tr('Target', 'ABILITY') + ' %s%s' % [count_str, bonus_type.get_term_tag()]
	else:
		return tr('Target', 'ABILITY') + ' %s%s' % [count_str, tr(bonus_type.name)]

func get_ability_tooltip(_card: Card) -> String:
	if count == 1:
		return tr('<term:fill> a random <term_lower:aspect_slot> in a <term:spot_upgrade> that produces %s.') % bonus_type.get_term_tag()
	else:
		return tr('<term:fill> %d random <term_lower:aspect_slot>s in <term:spot_upgrade>s that produce %s.') % [count, bonus_type.get_term_tag()]

func get_term() -> Term:
	return load('res://cards/abilities/terms/term_card_ability_target.tres')

func cast(_card: Card) -> void:
	var run := Utils.get_active_run()
	var stage := run.get_current_stage()

	var all_matching_slots: Array[AspectSlot] = []
	for spot in stage.get_spots():
		for recipe in spot.get_all_recipes():
			if recipe.spot_upgrade.granted_bonuses.get(bonus_type, 0) > 0:
				for slot in recipe.get_aspect_slots():
					all_matching_slots.append(slot)

	var slots_filled := [0]  # Array, so we can capture it.
	var error_shown := [false]  # Array, so we can capture it.
	for _i in range(count + run.get_var(RunVars.Var.FILL_ABILITY_BONUS)):
		stage.queue_action(func() -> void:
			var aspect_slots_availability := stage.get_aspect_slots_availability()
			var pool: Array[AspectSlot] = []
			for slot in all_matching_slots:
				if aspect_slots_availability.get(slot):
					pool.append(slot)
			if pool:
				var slot := stage.get_card_deck().get_random_state().pick(pool) as AspectSlot
				await stage.ensure_slot_visible(slot)
				await slot.animate_fill()
			else:
				if not error_shown[0]:
					if slots_filled[0]:
						GlobalUI.show_error(tr('Ran out of <term_lower:aspect_slot>s to fill after %d.') % slots_filled[0])
					else:
						GlobalUI.show_error(tr('No valid <term_lower:aspect_slot>s to fill.'))
					error_shown[0] = true
					await run.get_tree().create_timer(Utils.anim_duration(0.3)).timeout
		)

func estimate_power(_card_type: CardType) -> int:
	return 11 * count
