class_name CompanionRecipe_Tai
...
func _make_tooltip_text() -> String:
	var amount := AMOUNT_PER_SEASON * (Utils.get_active_run().get_current_season_index() + 1)
	var description := tr('Satisfy %d points of a random <term_lower:stage_goal>.') % amount
...
func _on_all_slots_filled() -> void:
	...
	stage.queue_action(func() -> void:
		...
		if (%AnimationPlayer as AnimationPlayer).current_animation != 'react':
			...
			(%AnimationPlayer as AnimationPlayer).queue('idle')
			GlobalAudioSystem.play(AK.EVENTS.SFX_ANIMAL_COMPANION_SEABEAM)
		...
		var points := AMOUNT_PER_SEASON * (run.get_current_season_index() + 1)

		for bonus_type in bonus_types:
			var clamped_points := mini(points, remaining_reqs.get(bonus_type, 0) as int)  # Auto-handles negatives.
			if clamped_points:
				run.gain_bonus(BonusGain.new(bonus_type, clamped_points, self))
...
func _get_companion() -> Companion:
	return load('res://companions/tai/companion_tai.tres') as Companion
