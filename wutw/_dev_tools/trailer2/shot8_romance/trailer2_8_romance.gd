...
func _ready() -> void:
	...
	GlobalSaveGame.mark_event_choice_seen(event, 0)
	GlobalSaveGame.mark_event_choice_seen(event, 1)
	GlobalSaveGame.mark_event_choice_seen(event, 2)
	...
	# Drag card to activate choice tooltip.
	#await get_tree().create_timer(3.5).timeout
	#Input.warp_mouse(Vector2(1000, 680))
